...
namespace Budapest
{
    ...
    public sealed class NotNotExpression : DiagnosticAnalyzer
    {
        private const string Category = "Performance";
        public const string DiagnosticId = nameof(NotNotExpression);
        ...
        {
            var syntax = context.Node as PrefixUnaryExpressionSyntax;

            if (syntax.Parent.IsKind(SyntaxKind.LogicalNotExpression))
                return;
            
            if (syntax.Operand.IsKind(SyntaxKind.LogicalNotExpression))
                context.MarkAsUnnecessary(Rule, syntax.OperatorToken, (syntax.Operand as PrefixUnaryExpressionSyntax).OperatorToken);
        }
    }
}
